    ...
    const getMaterials = async (pageNumber, itemsPerPage = 10) => {
      try {
        const respons = await axios.get(
          `/api/news?offset=${pageNumber}&limit=${itemsPerPage}`
        );
        ...
        <NewsBox>
          {materials.length === 0 ? (
            ...
            materials.map((material) => {
              ...
              return (
                <NewsCard key={material._id}>
                  <NewsBlueHeader />
                  <NewsBody>
                    <NewsImg src={material.imgUrl} alt={material.title} />
                    <NewsTitle>{material.title}</NewsTitle>
                    <NewsDescription>{material.text}</NewsDescription>
                    <NewsMore>
                      <NewsDate>{`${day}/${month}/${year}`}</NewsDate>
                      <NewsReadMoreLink href={material.url} target="_blank">
                        Read more
                      </NewsReadMoreLink>
                    </NewsMore>
                  </NewsBody>
